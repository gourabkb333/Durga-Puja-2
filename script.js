...
function switchRoute(route) {
  ...
  const emptyMsg = document.getElementById('routeEmpty');
  const fullscreenLink = document.getElementById('routeFullscreen');
  ...
  const src = SITE_CONFIG.mapRoutes[route];
  if (!src || src.includes('PASTE_')) {
    iframe.hidden = true;
    ...
    // Build a link to the full, freely interactive Google Maps view
    // from the same map ID used in the embed src.
    const midMatch = src.match(/[?&]mid=([^&]+)/);
    if (midMatch) {
      fullscreenLink.href = `https://www.google.com/maps/d/viewer?mid=${midMatch[1]}&hl=en`;
