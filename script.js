...
function setupRoutes() {
  const tabNorth = document.getElementById('tabNorth');
  const tabSouth = document.getElementById('tabSouth');

  tabNorth.addEventListener('click', () => switchRoute('north'));
  tabSouth.addEventListener('click', () => switchRoute('south'));
...
function switchRoute(route) {
  const tabNorth = document.getElementById('tabNorth');
  const tabSouth = document.getElementById('tabSouth');
  const openMapLink = document.getElementById('routeOpenMap');
  const listEl = document.getElementById('routeStopList');

  tabNorth.classList.toggle('active', route === 'north');
  tabSouth.classList.toggle('active', route === 'south');
  tabNorth.setAttribute('aria-selected', String(route === 'north'));
  tabSouth.setAttribute('aria-selected', String(route === 'south'));

  const routeData = SITE_CONFIG.mapRoutes[route];
  const mapUrl = routeData && routeData.mapUrl;
  const stops = (routeData && routeData.stops) || [];

  listEl.innerHTML = '';

  if (!mapUrl) {
    openMapLink.hidden = true;
    listEl.innerHTML = '<li class="route-empty-item">এই রুটের ম্যাপ এখনও যোগ করা হয়নি — config.js চেক করুন</li>';
    return;
  }
  ...
  if (stops.length === 0) {
    listEl.innerHTML = '<li class="route-empty-item">এখনও কোনো স্টপ যোগ করা হয়নি — config.js-এ যোগ করুন</li>';
    return;
  }
  ...
  stops.forEach((stopName) => {
    ...
    link.className = 'route-stop-item';
    link.href = mapUrl;
    link.target = '_blank';
    link.rel = 'noopener';
...
/* ---------- Background playback: best-effort mitigations ----------
   A hidden YouTube iframe is suspended by mobile browsers on screen
   lock / app switch — this is an OS/browser battery policy, not
   something a webpage can fully override. These are best-effort only:
   - Screen Wake Lock: delays auto-lock while the tab is open and
     visible (doesn't help once you switch apps or manually lock).
   - Media Session: gives Android browsers a proper lock-screen
     control widget on the occasions playback does survive.
   - Auto-resume: if the browser paused things while you were away,
     picks the song back up automatically when you return, instead of
     making you hit play again from scratch. ---------- */
let wakeLock = null;
let wasPlayingBeforeHidden = false;

function setupBackgroundResilience() {
  document.getElementById('playBtn').addEventListener('click', () => {
    setTimeout(requestWakeLock, 100); // after play state has updated
  });

  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      wasPlayingBeforeHidden = ytPlayer && ytPlayer.getPlayerState && ytPlayer.getPlayerState() === YT.PlayerState.PLAYING;
      releaseWakeLock();
    } else {
      if (wasPlayingBeforeHidden && ytPlayer && ytPlayer.getPlayerState) {
        const state = ytPlayer.getPlayerState();
        if (state !== YT.PlayerState.PLAYING) {
          ytPlayer.playVideo();
        }
...
async function requestWakeLock() {
  ...
  if (!ytPlayer || !ytPlayer.getPlayerState || ytPlayer.getPlayerState() !== YT.PlayerState.PLAYING) return;
  try {
    wakeLock = await navigator.wakeLock.request('screen');
  } catch (err) {
    // wake lock can be refused (e.g. low battery) — fine to ignore
  }
}
...
function setupMediaSession() {
  ...
  navigator.mediaSession.setActionHandler('previoustrack', () => ytPlayer && ytPlayer.previousVideo());
  navigator.mediaSession.setActionHandler('nexttrack', () => ytPlayer && ytPlayer.nextVideo());
}
