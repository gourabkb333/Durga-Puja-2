...
## 3. Add your pandal-hopping routes

A "পুজো পরিক্রমা" section shows two tabs — North Kolkata and South
Kolkata — each with a Google My Maps route through your club stops.

1. Go to **mymaps.google.com**, create a new map.
2. Add each club as a marker (or draw a route connecting them) — this
   is the "add stop" style workflow.
3. Click **Share**, make the map public, and copy the link — it'll
   look like `google.com/maps/d/viewer?mid=SOME_ID&...`.
4. Take just the `mid=` value and build your embed link:
   `https://www.google.com/maps/d/embed?mid=SOME_ID&hl=en`
5. Paste that into `config.js` under `mapRoutes.north` or
   `mapRoutes.south`. **Make sure North and South use two different
   map IDs** — right now they're set to the same one as a placeholder.

No API key or Google Cloud account needed.

## 4. Add your photo, dhak sound, and Durga face image
...
| Puja routes | Two tabs (North / South Kolkata) each showing a Google My Maps embed of your club stops — no API key |
